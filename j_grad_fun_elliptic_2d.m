function [G] = j_grad_fun_elliptic_2d(model,u,xi, deriv, vec)
%%% 2D Elliptic PDE Ex 5.2, cost, its gradient and Hessian
...
for i=I:-1:1
    % Forward solution
    [y,A] = solve_fun_elliptic_2d(model,u,xi(i,:),true);
    y = y(:);
    ...
    if (nargin<4)
        ...
        if (deriv==2)
            if (nargin>4)
                ...
                % Full hessian
                Ei = speye(ny);
                G(i,1:nu^2) = full(reshape(model.B'*inv(A')*Ei*model.My*inv(A)*Ei*model.B, 1, nu^2)); % B'*A^{-T}*My*A^{-1}*B
            end
        end
    end
end
