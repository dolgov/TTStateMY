function [x,R] = SEIRIEuler(dt,Nt,x0, u, Tnodes, beta,dL,dC,dR,dRC,dD,rho,rhop,alpha,S,Cfixed)
...
for it=1:Nt
    ...
    if (t<17)
        ...
        m = u*lagrange_interpolant(Tnodes, min(t, Tnodes(end)))';
        m = 1-m;
        m = reshape(m, 1, 1, 3);
        C = repmat(Cfixed(:,:,4), 1,1,size(x,3));
    ...
    if (abs(t-90)<1e-8)
        % Here we can compute the final R from the final J
        R = zeros(numel(beta), 1);
        for i=1:numel(beta)
            Ji = J(1+(i-1)*25:i*25, 1+(i-1)*25:i*25);
            Ji = full(Ji);
            Sigma = reshape(Ji, 5, 5, 5, 5);
            Sigma(:, 1, :, 2:3) = 0;
            Sigma = reshape(Sigma, 25, 25);
            R(i) = max(eig(-(Ji-Sigma)/Sigma));
        end
    end
end
end
